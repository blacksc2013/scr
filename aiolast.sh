...
# Prompt for MySQL root password and WordPress database details
read -sp 'Enter MySQL root password: ' MYSQL_ROOT_PASSWORD
echo
read -sp 'Enter desired WordPress database name: ' WORDPRESS_DB_NAME
echo
read -sp 'Enter desired WordPress database user: ' WORDPRESS_DB_USER
echo
read -sp 'Enter desired WordPress database user password: ' WORDPRESS_DB_PASSWORD
echo
...
if [ $? -ne 0 ]; then
    echo "Error creating MySQL database or user. Please check your MySQL root password and try again."
    exit 1
fi
